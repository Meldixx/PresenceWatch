...
let base=null,loadError='',unpatches=[],loadedVersion=LOADER;
...
function getId(p){if(!p||typeof p!=='object')return'';const v=p.user?.id??p.userId??p.user_id??p.targetUserId??p.profile?.user?.id??p.member?.user?.id??p.context?.user?.id??p.context?.userId??p.props?.user?.id??p.props?.userId;return v==null?'':String(v)}
...
function patchProfileMenu(){try{const Forms=vendetta.ui.components.Forms;const Lazy=vendetta.metro.findByProps('openLazy','hideActionSheet');const Simple=vendetta.metro.findByProps('showSimpleActionSheet');const patched=new WeakSet();const close=()=>{try{Lazy?.hideActionSheet?.()}catch{}};const act=id=>{close();toggle(id)};const looks=row=>!!row&&typeof row==='object'&&(typeof row?.props?.onPress==='function'||typeof row?.props?.label==='string'||/Row|Button|Item|Option|Entry/.test(String(row?.type?.name??row?.type?.displayName??'')));const makeRow=id=>R.createElement(Forms.FormRow,{key:'presencewatch-profile-row',label:has(id)?'✓ Удалить из PresenceWatch':'👁 Добавить в PresenceWatch',subLabel:has(id)?'Пользователь уже отслеживается':'Уведомлять, когда пользователь появится в сети',onPress:()=>act(id)});
function inject(tree,id){if(!tree||!valid(id)||!Forms?.FormRow)return tree;try{const node=makeRow(id);const container=vendetta.utils.findInReactTree(tree,n=>Array.isArray(n?.props?.children)&&n.props.children.some(looks));if(container){const children=container.props.children;if(!children.some(x=>x?.key==='presencewatch-profile-row')){const destructive=children.findIndex(x=>x?.props?.isDestructive||x?.props?.destructive);destructive>=0?children.splice(destructive,0,node):children.push(node)}return tree}return R.createElement(R.Fragment,null,tree,node)}catch{return tree}}
if(Lazy?.openLazy){unpatches.push(vendetta.patcher.before('openLazy',Lazy,args=>{try{const component=args?.[0],key=String(args?.[1]||''),props=args?.[2]||{},id=getId(props);if(!valid(id)||!/UserProfile/i.test(key)||typeof component?.then!=='function')return;component.then(mod=>{if(!mod||typeof mod.default!=='function'||patched.has(mod))return;patched.add(mod);unpatches.push(vendetta.patcher.after('default',mod,(a,result)=>{const idNow=getId(a?.[0])||id;return valid(idNow)?inject(result,idNow):result}))}).catch(()=>{})}catch{}}))}
