...
let patchTimer=null, channelInnerUnpatch=null, maintenanceTimer=null, summaryTimer=null;
...
const CHANGELOGS={
  '1.5.9':['Вернул расширенную статистику пользователя','Вернул блок активностей и расширенный Timeline','Вернул repeat-уведомления','Вернул поиск и сортировку Watchlist','Вернул cooldown, тихие часы и ежедневную сводку','Вернул Safe mode, проверку обновлений и полный Changelog','Сохранил совместимость старых PIN и системные Android-уведомления'],
  '1.5.8':['Совместимость PIN из старых версий','Автоматическая миграция PIN после успешного входа'],
  '1.5.7':['Системные уведомления через родной PushNotification Discord','Тест уведомления и диагностика'],
  '1.5.6':['Редактирование имени','Группы','Эмодзи и цвета','Управление оформлением пользователя'],
  '1.5.5':['Прямой main без hotfix-loader','Исправлен цикл React render'],
  '1.5.0':['PIN','Privacy','Drag/swipe','Pinned','Safe mode','Changelog'],
  '1.4.0':['Активности','Timeline','Страница пользователя','Группы','Пресеты','Сводки','Журнал ошибок']
};
...
function init(){
  ...
  S.notifyDiag={lastAttempt:0,lastSuccess:0,lastError:'',backend:'Не проверено',...(S.notifyDiag&&typeof S.notifyDiag==='object'?S.notifyDiag:{})};
...
function platform(id,p){
  let c=p?.clientStatus??p?.client_status??p?.presence?.clientStatus??p?.presence?.client_status;
  const parse=o=>{if(!o||typeof o!=='object')return'';let a=Object.entries(o).filter(([,v])=>ps(v)!=='offline').map(([k])=>k==='mobile'?'Mobile':k==='desktop'?'Desktop':k==='web'?'Web':k);return a.join(' + ')};
  let r=parse(c);if(r)return r;
  try{let s=pstore(),q=parse(s?.getClientStatus?.(String(id))??s?.getPresence?.(String(id))?.clientStatus);if(q)return q}catch{}
...
function safe(name,fn,fallback=null){
  ...
  try{return fn()}catch(e){
    logError(name,e);
    try{let c=(+S.safeMode.counts[name]||0)+1;S.safeMode={...S.safeMode,counts:{...S.safeMode.counts,[name]:c},disabled:c>=3&&S.safeMode.enabled?{...S.safeMode.disabled,[name]:true}:S.safeMode.disabled};if(c===3&&S.safeMode.enabled)toast(`PresenceWatch Safe mode: ${name} отключён`)}catch{}
    return fallback;
  }
}
...
function notificationBackend(){
  let a=discordPush();
  if(a?.presentLocalNotification)return['Discord PushNotification',body=>a.presentLocalNotification({alertBody:String(body)})];
  let m=N?.NativeModules?.PushNotificationAndroid;
  if(m?.presentLocalNotification)return['PushNotificationAndroid',body=>m.presentLocalNotification({alertBody:String(body)})];
...
function notify(body){
  ...
  try{
    ...
    b[1](body);
    S.notifyDiag={...S.notifyDiag,backend:b[0],lastSuccess:Date.now(),lastError:''};
    return true;
  }catch(e){logError('notifications',e);S.notifyDiag={...S.notifyDiag,lastError:String(e?.message||e)};toast('PresenceWatch: '+body);return false}
}
...
function add(id,a=''){
  id=String(id||'').trim();if(!valid(id)){toast('Некорректный Discord ID');return false}
  ...
  syncOrder();P.set(id,status(id));PL.set(id,platform(id));ensureStat(id);toast(`${displayName(id)} добавлен`);return true;
}
...
function scheduleRepeat(id){clearRepeat(id);let min=Math.max(0,+S.users?.[id]?.repeatMin||0);if(!min)return;REPEAT.set(id,setTimeout(()=>{REPEAT.delete(id);safe('repeat',()=>{if(has(id)&&!S.paused&&status(id)!=='offline'&&!prefs(id).muted&&!inQuietHours())notify(`🔔 ${displayName(id)} всё ещё в сети • ${platform(id)}`)})},min*60000))}
...
function event(p){safe('presence',()=>{
  ...
  if(S.paused)return;
  let now=Date.now(),acts=p?.activities??p?.presence?.activities;if(Array.isArray(acts))syncActs(id,acts,now,true);
  let n=ps(p?.status??p?.presence?.status??Object.values(p?.clientStatus||p?.client_status||{})[0])||status(id),o=P.has(id)?P.get(id):status(id),pl=platform(id,p);
  ...
  setStat(id,x);
  let nm=alias(id)||p?.user?.globalName||p?.user?.username||uname(id),entry={id,name:nm,at:now,from:o,to:n,platform:pl,kind:'status'};
...
function prime(){syncOrder();for(const id of tracked()){let s=status(id);P.set(id,s);PL.set(id,platform(id));let x=ensureStat(id);if(s!=='offline'&&!x.sessionStarted)setStat(id,{...x,sessionStarted:Date.now()});try{let a=pstore()?.getActivities?.(id)??pstore()?.getPresence?.(id)?.activities;if(Array.isArray(a))syncActs(id,a,Date.now(),false)}catch(e){logError('prime activities',e)}}}
function cleanup(){safe('cleanup',()=>{let cutoff=Date.now()-Math.max(1,+S.retentionDays||30)*86400000;S.history=S.history.filter(x=>+x.at>=cutoff).slice(0,1000);S.timeline=S.timeline.filter(x=>+x.at>=cutoff).slice(0,1500);S.errors=S.errors.slice(0,100)})}
...
function daily(force=false){safe('summary',()=>{if(!force&&!S.dailySummary.enabled)return;let now=new Date(),key=now.toDateString();if(!force&&(now.getHours()!==Math.max(0,Math.min(23,+S.dailySummary.hour||21))||S.dailySummary.lastDate===key))return;notify(summaryText());S.dailySummary={...S.dailySummary,lastDate:key}})}
...
function patchProfile(name){if(PATCHED.has(name))return true;return safe('profile-menu',()=>{let mod=vendetta.metro.findByName(name,false);if(!mod?.default)return false;let un=vendetta.patcher.after('default',mod,(args,ret)=>safe('profile-menu-runtime',()=>{let p=args?.[0]||{},id=String(p?.user?.id??p?.userId??'');if(valid(id))addProfileItem(getProfileItems(ret),id)}));UN.push(un);PATCHED.add(name);return true},false)}
function patchChannel(){let name='ChannelLongPressActionSheet';if(PATCHED.has(name))return true;return safe('channel-menu',()=>{let mod=vendetta.metro.findByName(name,false);if(!mod?.default)return false;let outer=vendetta.patcher.after('default',mod,(_,ret)=>safe('channel-runtime',()=>{let ch=ret?.props?.channel;if(!ch?.isDM?.())return;let id=String(ch?.getRecipientId?.()??'');if(!valid(id))return;try{channelInnerUnpatch?.()}catch{}channelInnerUnpatch=vendetta.patcher.after('type',ret,(_,tree)=>safe('channel-inner',()=>{let groups=vendetta.utils.findInReactTree(tree,n=>Array.isArray(n)&&String(n?.[0]?.type?.name||'')==='ActionSheetRowGroup');if(!Array.isArray(groups)||!groups.length)return;for(let g of groups){let children=Array.isArray(g?.props?.children)?g.props.children:[];if(children.some(x=>x?.key==='presencewatch-channel'))return;if(children[0]?.type){let row=R.cloneElement(children[0],{key:'presencewatch-channel',label:'PresenceWatch',subLabel:has(id)?'Отслеживается':'Не отслеживается',onPress:()=>toggle(id)});g.props.children=[children[0],row,...children.slice(1)];break}}}))}));UN.push(outer);PATCHED.add(name);return true},false)}
...
function Settings(){
  ...
  const[newPin,setNewPin]=R.useState('');
  ...
  const st={
    page:{padding:14,gap:12,paddingBottom:42},
    card:{backgroundColor:'#1d1f23',borderRadius:16,padding:14,gap:9},
    row:{flexDirection:'row',alignItems:'center',gap:8},grow:{flex:1},
    title:{color:'#fff',fontSize:22,fontWeight:'800'},head:{color:'#fff',fontSize:16,fontWeight:'800'},
    text:{color:'#e5e7eb',fontSize:14},muted:{color:'#8f96a3',fontSize:12,lineHeight:17},section:{color:'#8f96a3',fontSize:11,fontWeight:'800',textTransform:'uppercase'},
    input:{backgroundColor:'#111214',color:'#fff',borderRadius:10,padding:11},
    primary:{backgroundColor:'#5865F2',borderRadius:10,padding:10,alignItems:'center'},secondary:{backgroundColor:'#2a2d33',borderRadius:10,padding:9,alignItems:'center'},danger:{backgroundColor:'#4a292d',borderRadius:10,padding:9,alignItems:'center'},btn:{color:'#fff',fontWeight:'800'},
    chips:{flexDirection:'row',gap:6,flexWrap:'wrap'},chip:{backgroundColor:'#2a2d33',paddingHorizontal:8,paddingVertical:5,borderRadius:20},chipOn:{borderWidth:1,borderColor:'#5865F2'},
    statRow:{flexDirection:'row',gap:8,flexWrap:'wrap'},stat:{minWidth:'30%',flexGrow:1,backgroundColor:'#151619',borderRadius:10,padding:10,gap:3},statL:{color:'#767d89',fontSize:10,fontWeight:'700'},statV:{color:'#e8eaed',fontSize:12,fontWeight:'700'}
  };

  if(S.security.enabled&&!unlocked){
    return h(N.View,{style:[st.page,{flex:1,justifyContent:'center'}]},h(N.View,{style:st.card},
      h(N.Text,{style:st.title},'🔒 PresenceWatch'),
      h(N.Text,{style:st.muted},'Введите PIN для доступа к настройкам.'),
      h(N.TextInput,{style:st.input,value:pin,onChangeText:setPin,secureTextEntry:true,keyboardType:'numeric',maxLength:8,placeholder:'PIN',placeholderTextColor:'#666'}),
      h(N.Pressable,{style:st.primary,onPress:()=>{if(verifyPin(pin)){setUnlocked(true);setPin('')}else toast('Неверный PIN')}},h(N.Text,{style:st.btn},'Разблокировать'))
    ));
  }

  const masked=!!S.security.privacy;
  const shown=id=>masked?`${displayName(id).slice(0,1)}••••`:displayName(id);
  const shownId=id=>masked?`${String(id).slice(0,3)}••••••${String(id).slice(-3)}`:String(id);
  const groupName=id=>(S.groups||[]).find(g=>g.id===S.users[id]?.group)?.name||'Без группы';

  if(selected&&has(selected)){
    const id=selected,u=S.users[id],pf=prefs(id),x=stat(id),s=status(id),timeline=S.timeline.filter(e=>e.id===id).slice(0,80),acts=S.activities[id]?.items||[],curMs=s==='offline'?0:Math.max(0,Date.now()-(x.sessionStarted||Date.now()));
    return h(N.ScrollView,{contentContainerStyle:st.page,keyboardShouldPersistTaps:'handled'},
      ...
      h(N.View,{style:[st.card,{borderLeftWidth:4,borderLeftColor:u.color||COLORS[0]}]},
        ...
        h(N.Text,{style:st.muted},`${dot(s)} ${sl(s)} • ${platform(id)}`),
        h(N.Text,{style:st.muted},shownId(id))
      ),
      h(N.View,{style:st.card},
        h(N.Text,{style:st.head},'Статистика'),
        h(N.View,{style:st.statRow},
          h(N.View,{style:st.stat},h(N.Text,{style:st.statL},'СЕЙЧАС'),h(N.Text,{style:st.statV},s==='offline'?'Не в сети':dur(curMs))),
          h(N.View,{style:st.stat},h(N.Text,{style:st.statL},'ВСЕГО ОНЛАЙН'),h(N.Text,{style:st.statV},dur(x.totalMs||0))),
          h(N.View,{style:st.stat},h(N.Text,{style:st.statL},'ПРОШЛАЯ СЕССИЯ'),h(N.Text,{style:st.statV},dur(x.lastSessionMs||0))),
          h(N.View,{style:st.stat},h(N.Text,{style:st.statL},'LAST SEEN'),h(N.Text,{style:st.statV},s==='offline'?ago(x.lastSeen):'Сейчас')),
          h(N.View,{style:st.stat},h(N.Text,{style:st.statL},'СОБЫТИЙ'),h(N.Text,{style:st.statV},String(x.eventCount||0)))
        )
      ),
      h(N.View,{style:st.card},
        h(N.Text,{style:st.head},'Оформление и организация'),
        h(N.Text,{style:st.section},'Имя'),
        h(N.TextInput,{style:st.input,value:aliasValue,onChangeText:setAliasValue,placeholder:displayName(id),placeholderTextColor:'#666'}),
        h(N.Pressable,{style:st.primary,onPress:()=>{patchUser(id,{alias:aliasValue.trim()});toast('Имя сохранено')}},h(N.Text,{style:st.btn},'Сохранить имя')),
        h(N.Text,{style:st.section},'Группа'),
        h(N.View,{style:st.chips},
          h(N.Pressable,{style:[st.chip,!u.group&&st.chipOn],onPress:()=>patchUser(id,{group:''})},h(N.Text,{style:st.text},'Без группы')),
          ...(S.groups||[]).map(g=>h(N.Pressable,{key:g.id,style:[st.chip,u.group===g.id&&st.chipOn],onPress:()=>patchUser(id,{group:g.id})},h(N.Text,{style:st.text},g.name)))
        ),
        h(N.Text,{style:st.section},'Эмодзи / иконка'),
        h(N.View,{style:st.chips},...ICONS.map(ic=>h(N.Pressable,{key:ic,style:[st.chip,u.icon===ic&&st.chipOn],onPress:()=>patchUser(id,{icon:ic})},h(N.Text,{style:{fontSize:20}},ic)))),
        h(N.Text,{style:st.section},'Цвет'),
        h(N.View,{style:st.chips},...COLORS.map(c=>h(N.Pressable,{key:c,style:{width:38,height:38,borderRadius:19,backgroundColor:c,borderWidth:u.color===c?3:0,borderColor:'#fff'},onPress:()=>patchUser(id,{color:c})}))),
        h(N.View,{style:st.row},h(N.Text,{style:[st.text,st.grow]},'📌 Закрепить'),h(N.Switch,{value:!!u.pinned,onValueChange:v=>patchUser(id,{pinned:v})})),
        h(N.View,{style:st.row},h(N.Text,{style:[st.text,st.grow]},'⭐ Избранное'),h(N.Switch,{value:!!u.favorite,onValueChange:v=>patchUser(id,{favorite:v})}))
      ),
      h(N.View,{style:st.card},
        h(N.Text,{style:st.head},'Уведомления'),
        h(N.View,{style:st.chips},...PRESETS.map(pr=>h(N.Pressable,{key:pr.id,style:st.chip,onPress:()=>{setPrefs(id,pr.prefs);toast('Пресет: '+pr.name)}},h(N.Text,{style:st.text},pr.name)))),
        h(N.View,{style:st.row},h(N.Text,{style:[st.text,st.grow]},'Вход в сеть'),h(N.Switch,{value:pf.online,onValueChange:v=>setPrefs(id,{online:v})})),
        h(N.View,{style:st.row},h(N.Text,{style:[st.text,st.grow]},'Выход из сети'),h(N.Switch,{value:pf.offline,onValueChange:v=>setPrefs(id,{offline:v})})),
        h(N.View,{style:st.row},h(N.Text,{style:[st.text,st.grow]},'Смена статуса'),h(N.Switch,{value:pf.status,onValueChange:v=>setPrefs(id,{status:v})})),
        h(N.View,{style:st.row},h(N.Text,{style:[st.text,st.grow]},'Mute'),h(N.Switch,{value:pf.muted,onValueChange:v=>setPrefs(id,{muted:v})})),
        h(N.Pressable,{style:st.secondary,onPress:()=>{let i=REPEATS.indexOf(+u.repeatMin||0);patchUser(id,{repeatMin:REPEATS[(i+1)%REPEATS.length]})}},h(N.Text,{style:st.btn},`Повторное уведомление: ${u.repeatMin?u.repeatMin+' мин':'выкл.'}`))
      ),
      h(N.View,{style:st.card},
        h(N.Text,{style:st.head},'Активности'),
        ...(acts.length?acts.map((a,i)=>h(N.View,{key:i,style:st.stat},h(N.Text,{style:st.statV},`${a.label||activityType(a.type)}: ${a.name}`),a.details?h(N.Text,{style:st.muted},a.details):null,a.state?h(N.Text,{style:st.muted},a.state):null)):[h(N.Text,{key:'none',style:st.muted},'Нет видимых активностей')])
      ),
      h(N.View,{style:st.card},
        h(N.Text,{style:st.head},'Timeline'),
        ...(timeline.length?timeline.map((e,i)=>h(N.View,{key:i,style:st.stat},h(N.Text,{style:st.statV},e.kind==='status'?`${sl(e.from)} → ${sl(e.to)}`:e.kind==='activity_start'?`▶ ${e.activity?.name||''}`:`■ ${e.activity?.name||''}`),h(N.Text,{style:st.muted},new Date(e.at).toLocaleString()))):[h(N.Text,{key:'none',style:st.muted},'Пусто')])
      )
    );
  }

  let list=tracked(),q=search.trim().toLowerCase(),order=computeOrder(),oi=new Map(order.map((x,i)=>[x,i]));
  ...
  return h(N.ScrollView,{contentContainerStyle:st.page,keyboardShouldPersistTaps:'handled'},
    h(N.View,{style:st.card},
      ...
      S.seenVersion!==V?h(N.View,{style:{gap:5}},h(N.Text,{style:st.section},'Что нового'),...(CHANGELOGS[V]||[]).map((x,i)=>h(N.Text,{key:i,style:st.muted},`• ${x}`)),h(N.Pressable,{style:st.secondary,onPress:()=>S.seenVersion=V},h(N.Text,{style:st.btn},'Понятно'))):null
    ),
    h(N.View,{style:st.card},
      h(N.Text,{style:st.head},'Системные уведомления'),
      h(N.Text,{style:st.text},`Backend: ${notificationBackend()?.[0]||'не найден'}`),
      ...
      h(N.View,{style:st.row},h(N.Pressable,{style:[st.primary,st.grow],onPress:()=>notify('🔔 Тестовое уведомление PresenceWatch')},h(N.Text,{style:st.btn},'Тест уведомления')),h(N.Pressable,{style:[st.secondary,st.grow],onPress:openNotificationSettings},h(N.Text,{style:st.btn},'Настройки Android')))
    ),
    h(N.View,{style:st.card},
      h(N.Text,{style:st.head},'Watchlist'),
      ...
      h(N.TextInput,{style:st.input,value:search,onChangeText:setSearch,placeholder:'Поиск по имени / ID / группе',placeholderTextColor:'#666'}),
      h(N.Pressable,{style:st.secondary,onPress:()=>{let i=SORTS.indexOf(S.sortMode);S.sortMode=SORTS[(i+1)%SORTS.length]}},h(N.Text,{style:st.btn},`Сортировка: ${SORT_LABEL[S.sortMode]}`)),
      h(N.Text,{style:st.muted},'Свайп вправо — закрепить • влево — mute')
    ),
    ...list.map(id=>{
      const u=S.users[id],pf=prefs(id),open=cfg===id,s=status(id),x=stat(id);
      const card=h(N.View,{style:[st.card,{borderLeftWidth:4,borderLeftColor:u.color||COLORS[0]}]},
        h(N.View,{style:st.row},!masked&&avatar(id)?h(N.Image,{source:{uri:avatar(id)},style:{width:44,height:44,borderRadius:22}}):null,h(N.View,{style:st.grow},h(N.Text,{style:st.head},`${u.pinned?'📌 ':''}${u.favorite?'⭐ ':''}${u.icon||'👁'} ${shown(id)}`),h(N.Text,{style:st.muted},`${dot(s)} ${sl(s)} • ${groupName(id)}${pf.muted?' • 🔕':''}`))),
        h(N.View,{style:st.statRow},h(N.View,{style:st.stat},h(N.Text,{style:st.statL},'ПОСЛЕДНИЙ РАЗ'),h(N.Text,{style:st.statV},s==='offline'?ago(x.lastSeen):'Сейчас')),h(N.View,{style:st.stat},h(N.Text,{style:st.statL},'СОБЫТИЙ'),h(N.Text,{style:st.statV},String(x.eventCount||0)))),
        open?h(N.View,{style:{gap:8}},
          h(N.View,{style:st.row},h(N.Pressable,{style:[st.secondary,st.grow],onPress:()=>{moveOrder(id,-1);force(x=>x+1)}},h(N.Text,{style:st.btn},'↑ Выше')),h(N.Pressable,{style:[st.secondary,st.grow],onPress:()=>{moveOrder(id,1);force(x=>x+1)}},h(N.Text,{style:st.btn},'↓ Ниже'))),
          h(N.View,{style:st.row},h(N.Pressable,{style:[st.secondary,st.grow],onPress:()=>patchUser(id,{pinned:!u.pinned})},h(N.Text,{style:st.btn},u.pinned?'Открепить':'Закрепить')),h(N.Pressable,{style:[st.secondary,st.grow],onPress:()=>setPrefs(id,{muted:!pf.muted})},h(N.Text,{style:st.btn},pf.muted?'Unmute':'Mute')),h(N.Pressable,{style:[st.danger,st.grow],onPress:()=>remove(id)},h(N.Text,{style:st.btn},'Удалить')))
        ):null,
      ...
      return h(SwipeCard,{key:id,id,disabled:open},card);
    }),
    h(N.View,{style:st.card},
    ...
    h(N.View,{style:st.card},
      h(N.Text,{style:st.head},'Глобальные уведомления'),
      h(N.View,{style:st.row},h(N.Text,{style:[st.text,st.grow]},'По умолчанию: вход'),h(N.Switch,{value:!!S.defaults.online,onValueChange:v=>S.defaults={...S.defaults,online:v}})),
      h(N.View,{style:st.row},h(N.Text,{style:[st.text,st.grow]},'По умолчанию: выход'),h(N.Switch,{value:!!S.defaults.offline,onValueChange:v=>S.defaults={...S.defaults,offline:v}})),
      h(N.View,{style:st.row},h(N.Text,{style:[st.text,st.grow]},'По умолчанию: смена статуса'),h(N.Switch,{value:!!S.defaults.status,onValueChange:v=>S.defaults={...S.defaults,status:v}})),
      h(N.Pressable,{style:st.secondary,onPress:()=>{let i=COOLDOWNS.indexOf(+S.cooldownSec||0);S.cooldownSec=COOLDOWNS[(i+1)%COOLDOWNS.length]}},h(N.Text,{style:st.btn},`Cooldown: ${S.cooldownSec} сек`)),
      h(N.View,{style:st.row},h(N.Text,{style:[st.text,st.grow]},'Тихие часы'),h(N.Switch,{value:!!S.quiet.enabled,onValueChange:v=>S.quiet={...S.quiet,enabled:v}})),
      h(N.View,{style:st.row},h(N.Pressable,{style:[st.secondary,st.grow],onPress:()=>S.quiet={...S.quiet,start:(+S.quiet.start+1)%24}},h(N.Text,{style:st.btn},`С: ${String(S.quiet.start).padStart(2,'0')}:00`)),h(N.Pressable,{style:[st.secondary,st.grow],onPress:()=>S.quiet={...S.quiet,end:(+S.quiet.end+1)%24}},h(N.Text,{style:st.btn},`До: ${String(S.quiet.end).padStart(2,'0')}:00`)))
    ),
    h(N.View,{style:st.card},
      h(N.Text,{style:st.head},'Ежедневная сводка'),
      h(N.View,{style:st.row},h(N.Text,{style:[st.text,st.grow]},'Включена'),h(N.Switch,{value:!!S.dailySummary.enabled,onValueChange:v=>S.dailySummary={...S.dailySummary,enabled:v}})),
      h(N.Pressable,{style:st.secondary,onPress:()=>S.dailySummary={...S.dailySummary,hour:(+S.dailySummary.hour+1)%24}},h(N.Text,{style:st.btn},`Время: ${String(S.dailySummary.hour).padStart(2,'0')}:00`)),
      h(N.Pressable,{style:st.secondary,onPress:()=>daily(true)},h(N.Text,{style:st.btn},'Сводка сейчас'))
    ),
    h(N.View,{style:st.card},
      ...
      h(N.TextInput,{style:st.input,value:newPin,onChangeText:setNewPin,keyboardType:'numeric',secureTextEntry:true,maxLength:8,placeholder:S.security.enabled?'Новый PIN 4–8 цифр':'Создать PIN 4–8 цифр',placeholderTextColor:'#666'}),
      h(N.View,{style:st.row},h(N.Pressable,{style:[st.secondary,st.grow],onPress:()=>{if(!/^\d{4,8}$/.test(newPin))return toast('PIN должен быть 4–8 цифр');S.security={...S.security,enabled:true,pinHash:hash(newPin)};setNewPin('');toast('PIN сохранён')}},h(N.Text,{style:st.btn},S.security.enabled?'Сменить PIN':'Установить PIN')),S.security.enabled?h(N.Pressable,{style:[st.danger,st.grow],onPress:()=>{S.security={...S.security,enabled:false,pinHash:''};toast('PIN отключён')}},h(N.Text,{style:st.btn},'Отключить')):null)
    ),
    h(N.View,{style:st.card},
      h(N.Text,{style:st.head},'Safe mode'),
      h(N.View,{style:st.row},h(N.Text,{style:[st.text,st.grow]},'Автозащита модулей'),h(N.Switch,{value:!!S.safeMode.enabled,onValueChange:v=>S.safeMode={...S.safeMode,enabled:v}})),
      h(N.Text,{style:st.muted},`Отключено модулей: ${Object.keys(S.safeMode.disabled||{}).filter(k=>S.safeMode.disabled[k]).length}`),
      h(N.Pressable,{style:st.secondary,onPress:()=>{S.safeMode={enabled:true,disabled:{},counts:{}};toast('Safe mode сброшен')}},h(N.Text,{style:st.btn},'Сбросить Safe mode'))
    ),
    h(N.View,{style:st.card},
      h(N.Text,{style:st.head},'История / сервис'),
      h(N.Pressable,{style:st.secondary,onPress:()=>{S.retentionDays=S.retentionDays===7?30:S.retentionDays===30?90:7;cleanup()}},h(N.Text,{style:st.btn},`Хранить ${S.retentionDays} дней`)),
      ...
      h(N.Pressable,{style:st.danger,onPress:()=>S.errors=[]},h(N.Text,{style:st.btn},'Очистить журнал ошибок'))
    ),
    h(N.View,{style:st.card},
      h(N.Text,{style:st.head},'Обновления'),
      h(N.Text,{style:st.text},`Установлена: ${V}`),h(N.Text,{style:st.muted},`Последняя: ${S.update.latest||V}`),
      h(N.Pressable,{style:st.secondary,onPress:()=>checkUpdate(false)},h(N.Text,{style:st.btn},'Проверить обновления'))
    ),
    h(N.View,{style:st.card},
      h(N.Text,{style:st.head},'Changelog'),
      ...Object.entries(CHANGELOGS).map(([ver,items])=>h(N.View,{key:ver,style:{gap:3}},h(N.Text,{style:st.text},`v${ver}`),...items.map((x,i)=>h(N.Text,{key:i,style:st.muted},`• ${x}`))))
    )
  );
}

return{
  onLoad(){safe('load',()=>{cleanup();prime();F.subscribe('PRESENCE_UPDATE',event);startPatches();maintenanceTimer=setInterval(cleanup,3600000);summaryTimer=setInterval(()=>daily(false),60000);checkUpdate(true)})},
