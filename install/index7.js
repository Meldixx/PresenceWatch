(()=>{const V='1.5.8',S=vendetta.plugin.storage,R=vendetta.metro.common.React,N=vendetta.metro.common.ReactNative,F=vendetta.metro.common.FluxDispatcher,C=vendetta.metro.common.clipboard,P=new Map(),PL=new Map(),UN=[],PATCHED=new Set(),REP=new Map();let pt=null,ci=null,mt=null,st=null;
...
function hash(p){let s='PW:'+String(p||''),h=2166136261;for(let c of s){h^=c.charCodeAt(0);h=Math.imul(h,16777619)}return(h>>>0).toString(16)}
function legacyHash(p){let s=`PresenceWatch:${String(p||'')}:Meldix`,h=2166136261;for(let i=0;i<s.length;i++){h^=s.charCodeAt(i);h=Math.imul(h,16777619)}return(h>>>0).toString(16).padStart(8,'0')}
function verifyPin(p){let cur=String(S.security?.pinHash||''),next=hash(p);if(cur===next)return true;if(cur===legacyHash(p)){S.security={...S.security,pinHash:next};toast('PIN обновлён до нового формата');return true}return false}const blank=()=>({totalMs:0,lastSessionMs:0,sessionStarted:0,lastSeen:0,lastStatusAt:0,eventCount:0}),stat=id=>S.stats[id]||blank(),ensure=id=>{if(!S.stats[id])S.stats={...S.stats,[id]:blank()};return S.stats[id]},setstat=(id,x)=>S.stats={...S.stats,[id]:x},prefs=id=>{let p=S.users[id]?.prefs||{};return{online:p.online??S.defaults.online,offline:p.offline??S.defaults.offline,status:p.status??S.defaults.status,muted:!!p.muted}},patch=(id,x)=>{if(S.users[id])S.users={...S.users,[id]:{...S.users[id],...x}}},setprefs=(id,x)=>patch(id,{prefs:{...prefs(id),...x}});
...
function Settings(){
  ...
  return h(N.ScrollView,{contentContainerStyle:st.page,keyboardShouldPersistTaps:'handled'},
    ...
    h(N.View,{style:st.card},
      ...
      h(N.Text,{style:st.muted},'v1.5.8: восстановлена совместимость PIN из старых версий; после входа PIN автоматически мигрирует. Системные уведомления из 1.5.7 сохранены.')
    )
  );
}
return{
