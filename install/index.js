...
function patchProfileMenu(){
  try{
    ...
    if(!sheets?.openLazy)throw new Error('openLazy not found');
    unpatchProfile=vendetta.patcher.before('openLazy',sheets,([sheet,args])=>{
      try{
        if(String(sheet)!=='UserProfileActionSheet')return;
        if(!args||typeof args!=='object')return;
        const id=String(args?.user?.id??args?.userId??'');
        if(!valid(id))return;
        if(!Array.isArray(args.buttons))args.buttons=[];
        if(args.buttons.some(b=>b?.__presencewatch||String(b?.text||'').includes('PresenceWatch')))return;
        const item={__presencewatch:true,text:has(id)?'✓ Удалить из PresenceWatch':'👁 Добавить в PresenceWatch',onPress:()=>toggle(id)};
        const destructive=args.buttons.findIndex(b=>b?.isDestructive||b?.destructive||/блок|block|report/i.test(String(b?.text||'')));
        destructive>=0?args.buttons.splice(destructive,0,item):args.buttons.push(item);
      }catch(e){toast('PresenceWatch: ошибка меню профиля')}
    });
  }catch(e){toast('PresenceWatch: меню профиля недоступно')}
}
...
function Settings(){
  ...
  return h(N.ScrollView,{contentContainerStyle:st.page,keyboardShouldPersistTaps:'handled'},h(N.View,{style:st.hero},h(N.Text,{style:st.title},'PresenceWatch'),h(N.Text,{style:st.hint},`VERSION ${V} • Работает`),h(N.View,{style:st.metricRow},h(N.View,{style:st.metric},h(N.Text,{style:st.metricNum},String(ids.length)),h(N.Text,{style:st.metricLab},'Отслеживается')),h(N.View,{style:st.metric},h(N.Text,{style:st.metricNum},String(online)),h(N.Text,{style:st.metricLab},'Онлайн')),h(N.View,{style:st.metric},h(N.Text,{style:st.metricNum},String((S.history||[]).length)),h(N.Text,{style:st.metricLab},'Событий'))),h(N.Text,{style:st.hint},`Запущен: ${ago(started)} • Последнее событие: ${S.lastEventAt?ago(S.lastEventAt):'ещё не было'}`)),h(N.View,{style:st.card},h(N.Text,{style:st.head},'Добавить пользователя'),h(N.TextInput,{style:st.input,value:input,onChangeText:setInput,keyboardType:'numeric',placeholder:'Discord ID',placeholderTextColor:'#666b74'}),h(N.Pressable,{style:st.primary,onPress:()=>{if(add(input))setInput('')}},h(N.Text,{style:st.btn},'＋ Добавить')),h(N.Text,{style:st.hint},'Или: профиль пользователя → ⋯ → Добавить в PresenceWatch')),...(cards.length?cards:[h(N.View,{key:'empty',style:st.card},h(N.Text,{style:st.hint},'Список пока пуст.'))]),h(N.View,{style:st.card},h(N.Text,{style:st.head},'Уведомления'),h(N.View,{style:st.switch},h(N.Text,{style:st.text},'Уведомлять о выходе'),h(N.Switch,{value:!!S.notifyOffline,onValueChange:v=>S.notifyOffline=v})),h(N.View,{style:st.switch},h(N.Text,{style:st.text},'Online / Idle / DND'),h(N.Switch,{value:!!S.notifyStatusChanges,onValueChange:v=>S.notifyStatusChanges=v})),h(N.Pressable,{style:st.secondary,onPress:()=>notify('🟢 Тестовое уведомление PresenceWatch')},h(N.Text,{style:st.btn},'🔔 Проверить уведомление'))),h(N.View,{style:st.card},h(N.Text,{style:st.head},'Свои тексты уведомлений'),h(N.Text,{style:st.hint},'Переменные: {name} {id} {status} {platform} {time} {duration}'),h(N.TextInput,{style:st.inputMulti,value:to,onChangeText:setTo,multiline:true}),h(N.TextInput,{style:st.inputMulti,value:tf,onChangeText:setTf,multiline:true}),h(N.TextInput,{style:st.inputMulti,value:ts,onChangeText:setTs,multiline:true}),h(N.Pressable,{style:st.primary,onPress:()=>{S.templates={online:to||DEF.online,offline:tf||DEF.offline,status:ts||DEF.status};toast('Тексты сохранены')}},h(N.Text,{style:st.btn},'Сохранить шаблоны'))),h(N.View,{style:st.card},h(N.Text,{style:st.head},'Экспорт / импорт'),h(N.View,{style:st.buttons},h(N.Pressable,{style:[st.secondary,{flex:1}],onPress:exp},h(N.Text,{style:st.btn},'Экспорт')),h(N.Pressable,{style:[st.secondary,{flex:1}],onPress:imp},h(N.Text,{style:st.btn},'Импорт')))),h(N.View,{style:st.card},h(N.Text,{style:st.head},'История статусов'),...(hist.length?hist:[h(N.Text,{key:'none',style:st.hint},'История появится после изменения статуса.')]),hist.length?h(N.Pressable,{style:st.danger,onPress:()=>{S.history=[];toast('История очищена')}},h(N.Text,{style:st.btn},'Очистить историю')):null),h(N.Text,{style:st.hint},'Invisible отображается как Offline. PresenceWatch видит только presence-события, которые получает Discord-клиент.'));
}

return{onLoad(){started=Date.now();prime();F.subscribe('PRESENCE_UPDATE',event);patchProfileMenu()},onUnload(){try{F.unsubscribe('PRESENCE_UPDATE',event)}catch{};try{unpatchProfile?.()}catch{};P.clear();PL.clear()},settings:Settings};
})()
