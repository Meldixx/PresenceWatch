...
function getProfileItems(ret){
  try{
    ...
    if(Array.isArray(items)){if(Array.isArray(items[0]))return items[0];return items}
    items=ret?.props?.children?.props?.items;
    if(Array.isArray(items)){if(Array.isArray(items[0]))return items[0];return items}
    const node=vendetta.utils.findInReactTree(ret,n=>Array.isArray(n?.props?.items));
...
function addProfileItem(items,id){
  ...
  const item={id:'presencewatch-profile',label:'PresenceWatch',subLabel:has(id)?'Отслеживается':'Не отслеживается',action:()=>toggle(id)};
  const di=items.findIndex(x=>x?.variant==='destructive'||x?.isDestructive);
  di>=0?items.splice(di,0,item):items.push(item);
  return true;
}
function patchProfileNamed(name){
  if(PATCHED.has(name))return true;
  let mod=null;try{mod=vendetta.metro.findByName(name,false)}catch{}
  if(!mod?.default)return false;
  try{
    const un=vendetta.patcher.after('default',mod,(args,ret)=>{
      try{
        const props=args?.[0]||{};
        const id=String(props?.user?.id??props?.userId??'');
        if(!valid(id))return;
        if(addProfileItem(getProfileItems(ret),id))S.debug.profileInjected=true;
...
function findActionGroups(tree){
  try{return vendetta.utils.findInReactTree(tree,n=>Array.isArray(n)&&String(n?.[0]?.type?.name||'')==='ActionSheetRowGroup')}catch{return null}
}
function rowList(group){let c=group?.props?.children;return Array.isArray(c)?c:(c?[c]:[])}
function findEyeIcon(groups){
  try{for(const g of groups){for(const row of rowList(g)){const label=String(row?.props?.label??'');const iconName=String(row?.props?.icon?.props?.IconComponent?.name??row?.props?.icon?.type?.name??'');if(/прочитан|read/i.test(label)||/eye/i.test(iconName))return row?.props?.icon}}}catch{}
  return null;
}
function patchChannelLongPress(){
  const name='ChannelLongPressActionSheet';
  if(PATCHED.has(name))return true;
  let mod=null;try{mod=vendetta.metro.findByName(name,false)}catch{}
  if(!mod?.default)return false;
  try{
    const outer=vendetta.patcher.after('default',mod,(_,ret)=>{
      try{
        const ch=ret?.props?.channel;
        if(!ch?.isDM?.())return;
        const id=String(ch?.getRecipientId?.()??'');
        if(!valid(id))return;
        try{channelInnerUnpatch?.()}catch{}
        channelInnerUnpatch=vendetta.patcher.after('type',ret,(_,component)=>{
          try{
            const groups=findActionGroups(component);
            if(!Array.isArray(groups)||!groups.length){S.debug.channelInjected=false;return}
            if(groups.some(g=>rowList(g).some(x=>x?.key==='presencewatch-channel-row'))){S.debug.channelInjected=true;return}
            const gi=groups.findIndex(g=>g?.type&&rowList(g).length);
            if(gi<0){S.debug.channelInjected=false;return}
            const baseGroup=groups[gi];
            const children=rowList(baseGroup);
            const template=children[0];
            if(!template?.type){S.debug.channelInjected=false;return}
            const eyeIcon=findEyeIcon(groups)??template?.props?.icon;
            const row=R.cloneElement(template,{key:'presencewatch-channel-row',label:'PresenceWatch',subLabel:has(id)?'Отслеживается':'Не отслеживается',icon:eyeIcon,onPress:()=>{toggle(id);try{ret?.props?.onClose?.()}catch{}}});
            const next=[...children];
            next.splice(Math.min(1,next.length),0,row);
            groups[gi]=R.cloneElement(baseGroup,{children:next});
            S.debug.channelInjected=true;
          }catch(e){S.debug.channelInjected=false;S.debug.lastPatchError=String(e?.message||e)}
        });
      }catch(e){S.debug.lastPatchError=String(e?.message||e)}
    });
    UN.push(outer);PATCHED.add(name);return true;
  }catch(e){S.debug.lastPatchError=String(e?.message||e);return false}
}
...
function Settings(){
  ...
  const[,tick]=R.useState(0),[input,setInput]=R.useState(''),[editId,setEditId]=R.useState(null),[editValue,setEditValue]=R.useState(''),[delId,setDelId]=R.useState(null),[to,setTo]=R.useState(S.templates.online),[tf,setTf]=R.useState(S.templates.offline),[ts,setTs]=R.useState(S.templates.status);
  R.useEffect(()=>{let t=setInterval(()=>tick(x=>x+1),1000);return()=>clearInterval(t)},[]);
  const ids=tracked(),online=ids.filter(id=>status(id)!=='offline').length;
  ...
  return h(N.ScrollView,{contentContainerStyle:st.page,keyboardShouldPersistTaps:'handled'},h(N.View,{style:st.hero},h(N.Text,{style:st.title},'PresenceWatch'),h(N.Text,{style:st.hint},`VERSION ${V} • ● Активен`),h(N.View,{style:st.metricRow},h(N.View,{style:st.metric},h(N.Text,{style:st.metricNum},String(ids.length)),h(N.Text,{style:st.metricLab},'Отслеживается')),h(N.View,{style:st.metric},h(N.Text,{style:st.metricNum},String(online)),h(N.Text,{style:st.metricLab},'Онлайн')),h(N.View,{style:st.metric},h(N.Text,{style:st.metricNum},String((S.history||[]).length)),h(N.Text,{style:st.metricLab},'Событий'))),h(N.Text,{style:st.hint},`Запущен: ${ago(started)} • Последнее событие: ${S.lastEventAt?ago(S.lastEventAt):'ещё не было'}`),h(N.Text,{style:st.hint},`Меню профиля: ${S.debug.profileInjected?'подключено':'ожидание'}`),h(N.Text,{style:st.hint},`Long press: ${S.debug.channelInjected?'подключено':'ожидание открытия меню'}`)),h(N.View,{style:st.card},h(N.Text,{style:st.section},'Watchlist'),h(N.Text,{style:st.head},'Добавить пользователя'),h(N.TextInput,{style:st.input,value:input,onChangeText:setInput,keyboardType:'numeric',placeholder:'Discord ID',placeholderTextColor:'#666b74'}),h(N.Pressable,{style:st.primary,onPress:()=>{if(add(input))setInput('')}},h(N.Text,{style:st.btn},'＋ Добавить')),h(N.Text,{style:st.hint},'Также можно добавить через ⋯ профиля или долгим нажатием на ЛС.')),...cards,h(N.View,{style:st.card},h(N.Text,{style:st.section},'Уведомления'),h(N.View,{style:st.switch},h(N.View,{style:st.grow},h(N.Text,{style:st.text},'Выход из сети'),h(N.Text,{style:st.hint},'Уведомить при переходе в Offline')),h(N.Switch,{value:!!S.notifyOffline,onValueChange:v=>S.notifyOffline=v})),h(N.View,{style:st.switch},h(N.View,{style:st.grow},h(N.Text,{style:st.text},'Online / Idle / DND'),h(N.Text,{style:st.hint},'Уведомлять о смене активного статуса')),h(N.Switch,{value:!!S.notifyStatusChanges,onValueChange:v=>S.notifyStatusChanges=v})),h(N.Pressable,{style:st.secondary,onPress:()=>notify('🟢 Тестовое уведомление PresenceWatch')},h(N.Text,{style:st.btn},'🔔 Проверить уведомление'))),h(N.View,{style:st.card},h(N.Text,{style:st.section},'Свои тексты уведомлений'),h(N.Text,{style:st.hint},'Переменные: {name} {id} {status} {platform} {time} {duration}'),h(N.TextInput,{style:st.inputMulti,value:to,onChangeText:setTo,multiline:true}),h(N.TextInput,{style:st.inputMulti,value:tf,onChangeText:setTf,multiline:true}),h(N.TextInput,{style:st.inputMulti,value:ts,onChangeText:setTs,multiline:true}),h(N.Pressable,{style:st.primary,onPress:()=>{S.templates={online:to||DEF.online,offline:tf||DEF.offline,status:ts||DEF.status};toast('Тексты сохранены')}},h(N.Text,{style:st.btn},'Сохранить шаблоны'))),h(N.View,{style:st.card},h(N.Text,{style:st.section},'Экспорт / импорт'),h(N.View,{style:st.buttons},h(N.Pressable,{style:[st.secondary,{flex:1}],onPress:exp},h(N.Text,{style:st.btn},'Экспорт')),h(N.Pressable,{style:[st.secondary,{flex:1}],onPress:imp},h(N.Text,{style:st.btn},'Импорт')))),h(N.View,{style:st.card},h(N.Text,{style:st.section},'История'),...(hist.length?hist:[h(N.Text,{key:'empty',style:st.hint},'История появится после изменения статуса.')]),hist.length?h(N.Pressable,{style:st.danger,onPress:()=>{S.history=[];toast('История очищена')}},h(N.Text,{style:st.btn},'Очистить историю')):null),h(N.Text,{style:st.hint},'Invisible отображается как Offline. PresenceWatch видит только presence-события, которые получает твой Discord-клиент.'));
}

return{onLoad(){started=Date.now();prime();F.subscribe('PRESENCE_UPDATE',event);startMenuPatches()},onUnload(){try{F.unsubscribe('PRESENCE_UPDATE',event)}catch{};if(patchTimer){clearInterval(patchTimer);patchTimer=null}try{channelInnerUnpatch?.()}catch{};channelInnerUnpatch=null;for(const u of UN.splice(0))try{u?.()}catch{};PATCHED.clear();P.clear();PL.clear()},settings:Settings}
})()
