...
function addProfileItem(items,id){
  ...
  const item={id:'presencewatch-profile',label:has(id)?'✓ Удалить из PresenceWatch':'👁 Добавить в PresenceWatch',action:()=>toggle(id)};
      ...
      try{
        ...
        channelInnerUnpatch=vendetta.patcher.after('type',ret,(_,component)=>{
          try{
            ...
            if(groups.some(g=>g?.key==='presencewatch-channel-group')){S.debug.channelInjected=true;return}
            const gt=groups.find(g=>g?.type&&Array.isArray(g?.props?.children)&&g.props.children.length);
            const rt=gt?.props?.children?.find(x=>x?.type);
            if(!gt?.type||!rt?.type){S.debug.channelInjected=false;return}
            const row=R.cloneElement(rt,{key:'presencewatch-channel-row',label:has(id)?'✓ Удалить из PresenceWatch':'👁 Добавить в PresenceWatch',onPress:()=>{toggle(id);try{ret?.props?.onClose?.()}catch{}}});
            const group=R.createElement(gt.type,{key:'presencewatch-channel-group',hasIcons:gt?.props?.hasIcons??true},row);
            groups.push(group);
...
function tryInstallMenuPatches(){
  patchProfileNamed('UserProfileOverflowMenu');
  patchProfileNamed('BotUserProfileOverflowMenu');
  patchChannelLongPress();
...
return{
  ...
  onUnload(){try{F.unsubscribe('PRESENCE_UPDATE',event)}catch{};if(patchTimer){clearInterval(patchTimer);patchTimer=null}try{channelInnerUnpatch?.()}catch{};channelInnerUnpatch=null;for(const u of UN.splice(0))try{u?.()}catch{};PATCHED.clear();P.clear();PL.clear()},
  settings:Settings
}
})()
